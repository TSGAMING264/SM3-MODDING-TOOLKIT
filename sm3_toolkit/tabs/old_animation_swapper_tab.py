#!/usr/bin/env python3
# ============================================================
# SM3 ANIMATION SWAPPER RELEASE v1.10
...
def resource_path(filename: str) -> Path:
    """Find bundled animation swapper resources from the toolkit resource folder."""
    candidates = [
        paths.OLD_ANIMATION_SWAPPER_DIR / filename,
        paths.RESOURCE_ROOT / filename,
        Path(__file__).resolve().parent / filename,
    ]
    if getattr(sys, "frozen", False):
        bundle_dir = Path(getattr(sys, "_MEIPASS", Path(sys.executable).resolve().parent))
        candidates.insert(0, bundle_dir / "SM3_EXTRACTOR_FINAL" / "12_OLD_ANIMATION_SWAPPER" / filename)
...
class OldAnimationSwapperTab(ttk.Frame):
    ...
    def swing_test_lab(self):
        ...
        win = tk.Toplevel(self.root)
        win.title("SM3 ANIMATION SWAPPER RELEASE v1.10 - Swing / Dive / Jump Lab")
        apply_app_icon(win)
        ...
        ttk.Label(win, text=note, wraplength=820, foreground="#C7CCD6", padding=8).pack(fill="x")
